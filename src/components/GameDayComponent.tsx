...
import MatchesListComponent from './MatchesListComponent'
...
export default function GameDayComponent({ gameDay }: { gameDay: GameDaySummaryData | SerieDetailsGameDayData }) {
  const [open, setOpen] = useState(false)
  const { trigger: fetchData, data } = useRequestMutation<{
  ...
  return (
    <>
      <Stack direction='row'>
        <IconButton aria-label='expand row' size='small' onClick={async () => await handleOpen(!open)}>
        ...
        {data && (
          <Grid container spacing={1}>
            <Grid item xs={12} sm={6}>
              <GameDayPlayerStatsComponent title='Atletas' playersStats={data?.gameDayPlayerStats} />
            </Grid>
            <Grid item xs={12} sm={6}>
